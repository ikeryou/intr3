...
function init() {
  ...
  $('.mv .inner').each(function(i,e) {
    ...
    tgList.push({
      el:$(e),
      x:random(-size * range, size * range),
      y:random(-size * range, size * range),
      z:random(-size * range, size * range),
      // x:random(sw * range * 0.5, sw * range),
      // y:random(sw * range * 0.5, sw * range),
      // z:random(sw * range * 0.5, sw * range),
      distX:0,
      distY:0,
      scale:1,
      scaleOffset:random(20, 40) * 2,
      scaleOffset2:random(5, 30) * 1,
      fl:400,
      ease:random(0.01, 0.1) * 0.5,
      speedX:speedX,
      speedY:speedY,
      speedZ:speedZ,
      angle1:random(0, 360),
...
function update() {
  ...
  for(var i = 0; i < num; i++) {
    ...
    p3 = p2 + o.scale * o.scaleOffset;

    x = o.distX;
    y = o.distY;

    // color1 = chroma.mix(o.color1, o.color2, map(Math.sin(radian(o.angle1 * 10)), 0, 1, -1, 1)).css();
    color1 = chroma.mix(o.color3, o.color4, 1).css();

    cc = chroma.mix(o.color2, o.color3, map(Math.sin(radian(o.angle2)), 0, 1, -1, 1))

    color2 = chroma.mix(cc, o.color1, map(Math.sin(radian(o.angle1)), 0, 1, -1, 1)).css();
    color3 = chroma.mix(o.color3, o.color4, 1).css();
    // color3 = o.color4;
